...
서버에서 약속된 명령어로 구동 중인 서버를 컨트롤 할 수 있도록 별도의 쓰레드 할당. (별도의 클래스로 구현 , ConsoleManager.h , ConsoleManager.cpp)

### 유니티 클라이언트 씬 분할 자동화 및 로드

SceneManager.LoadSceneAsync(sceneName, LoadSceneMode.Additive); 를 활용해서 유니티 클라이언트가 아트리소스를 추척하고 있는 캐릭터의 주변만 로드하도록 함.
